...
class Motor(object):
    master_pulse = 0
    is_running = False
    def __init__(self, mtr_id, step_pin, dir_pin, ena_pin, steps_per_rev=200*4):
    ...
    def set_dir(self):
        GPIO.output(self.dir_pin, GPIO.HIGH) if np.sign(self.target_pos - self.current_pos) >= 0 else GPIO.output(self.dir_pin, GPIO.LOW)
...
class actuator_stepper(object):
    def __init__(self):
        ...
        self.Mtr_0 = Motor(mtr_id=0, step_pin=step_pin_0, ena_pin=ena_pin_0, dir_pin=dir_pin_0)
        self.Mtr_1 = Motor(mtr_id=1, step_pin=step_pin_1, ena_pin=ena_pin_1, dir_pin=dir_pin_1)
...
#        print("STARTED thread target_steps")
        while True:
            if (not target_steps_queue.empty()) and (not Motor.is_running):
                target_steps = target_steps_queue.get()
    ...
    #                    print("Steps_to_Move: ", steps_to_move)
                    ...
                    steps_to_move_temp = np.where(steps_to_move_temp==0, 1, steps_to_move_temp)
                    R_vals = max(np.abs(steps_to_move_temp))/np.abs(steps_to_move_temp)
                except:
                    print("DEBUG")
                    print(steps_to_move)
                    R_vals = [1, 1]
    ...
    def move_stpr(self, motors):
        ...
        # Set Motor pins High
        for motor in motors:
            if (motor.target_pos != motor.current_pos) & (Motor.master_pulse >= motor.R_temp-1):
                motor.step_low()
