...
class robot(object):
    def __init__(self, actuator, kinematics):
        ...
        t1 = threading.Thread(target=self.actuator.set_target_steps, args=(self.steps_to_move_queue,))
        t1.start()
        self.initiate_actuators(x=0, y=186.6)
        print("Initiating ROBOT")
        self.current_pos_xy = [0, 187]
    ...
    def initiate_actuators(self, x, y):
        ...
        self.steps_to_move_queue.put([0, 0])
    ...
    def move_robot(self, x, y):
        ...
        target_position = [x, y]
        linear_segments = self.bresenham_line(self.current_pos_xy[0], self.current_pos_xy[1], target_position[0], target_position[1])
        
        for xy_points in linear_segments:
            x, y = xy_points
            ...
            self.steps_to_move_queue.put(steps_to_move)
            time.sleep(150e-6)
